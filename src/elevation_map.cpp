    ...
    for (std::size_t linear_index = 0; linear_index < rows_ * cols_; ++linear_index)
    {
        ...
        if (has_valid_point)
        {
            ...
            else
            {
                ...
                maps_[UNCERTAINTY](row, col) = 1.0f;
            }
        }
    }
...
void ElevationMap::computeRoughnessMap()
{
    ...
    for (int i = half_window; i < static_cast<int>(rows_) - half_window; i++)
    {
        for (int j = half_window; j < static_cast<int>(cols_) - half_window; j++)
        {
            ...
            // Use Welford Variance Computation
            // Not adopt One Pass variance computation algorithm because it is not stable
            for (int wi = -half_window; wi <= half_window; wi++)
            {
                for (int wj = -half_window; wj <= half_window; wj++)
                {
            ...
            if (count > 0)
            {
                maps_[ROUGHNESS](i, j) = std::sqrt(s / static_cast<float>(count));
            }
            else
            {
                maps_[ROUGHNESS](i, j) = std::numeric_limits<float>::quiet_NaN();
...
void ElevationMap::computeTraversabilityMap()
{
    ...
    for (int i = 0; i < static_cast<int>(rows_); i++)
    {
        for (int j = 0; j < static_cast<int>(cols_); j++)
        {
            ...
            float slope_term = slope_weight_ * maps_[SLOPE](i, j) / slope_critical_;
            float step_height_term = step_height_weight_ * maps_[STEP_HEIGHT](i, j) / step_height_critical_;
            float roughness_term = roughness_weight_ * maps_[ROUGHNESS](i, j) / roughness_critical_;
